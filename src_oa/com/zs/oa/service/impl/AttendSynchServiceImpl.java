...
@Service
public class AttendSynchServiceImpl implements IAttendSynchService {
   ...
   // @Autowired
    ...
    @Override
    public String getDataFormAttend(String attendIp, int port,
	    String secretKey, String charSet) throws Exception {
	String answer = null;
	try {

	    FaceId tcpClient = new FaceId(attendIp, port);
	    String sdfend = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
		    .format(new Date());
	    String startTime = dao.getStart_Time();

	    if (startTime != null && (!startTime.equals("false"))) {
		String Command = "GetRecord(start_time=\"" + startTime
			+ "\" end_time=\"" + sdfend + "\")";
    ...
    @Override
    public String getAttDataWithoutStartTime(String attendIp, int port,
	    String secretKey, String charSet) throws Exception {
	...
	// 时间
	Pattern p = Pattern
		.compile("([0-9]{3}[1-9]|[0-9]{2}[1-9][0-9]{1}|[0-9]{1}[1-9][0-9]{2}|[1-9][0-9]{3})-(((0[13578]|1[02])-(0[1-9]|[12][0-9]|3[01]))|((0[469]|11)-(0[1-9]|[12][0-9]|30))|(02-(0[1-9]|[1][0-9]|2[0-9])))\\s\\d+:\\d+:\\d+");
	...
	for (int i = 0; i < listime.size(); i++) {
	    // System.out.println(listime.get(i)+" "+listid.get(i)+" "+listname.get(i));
	    Employee employee = new Employee(listime.get(i), listid.get(i),
		    listname.get(i));
	    list.add(employee);

	}
	log.info("listtime size " + listime.size());
	return list;
    }

    @Override
    public String insertDataWithoutJudge(List<Employee> list, String attendIp)
	    throws Exception {
	// TODO Auto-generated method stub
	...
	try {
		...
		if (user_id != null && (!"".equals(user_id))
			...
			dao.insertAttendRecord(user_id, register_type,
				em.getTime(), attendIp, "", duty_type, 0, 0);
    ...
    @Override
    public String insertEmployee(List<Employee> list, String attendIp)
	    throws Exception {
	// TODO Auto-generated method stub
	...
	try {
		...
		if (user_id != null && (!"".equals(user_id))
			...
			if (register_type != null
				&& !register_type.equals("false")) {
			    if (register_type.equals("2")) {
				String count2 = dao.selectMinValueForPM(
					user_id, "2");
				if (count2.equals("1")) {
				    String result = dao.deleteMinValueForPM(
					    user_id, "2", "");
				    if (result.equals("true")) {
					dao.insertAttendRecord(user_id,
						register_type, em.getTime(),
						attendIp, "", duty_type, 0, 0);
				    }
				    dao.insertAttendRecord(user_id,
					    register_type, em.getTime(),
					    attendIp, "", duty_type, 0, 0);
				...
				String count3 = dao.selectMinValueForPM(
					user_id, "4");
				if (count3.equals("1")) {
				    String result = dao.deleteMinValueForPM(
					    user_id, "4", "");
    ...
    @Override
    public String insertEmployeeData(List<EmployDto> list, String address)
	    throws Exception {
	// TODO Auto-generated method stub
	...
	try {
		...
		duty_type = dao.queryDuty_Type(user_id);// 从人事扩展表中获取排版类型
		if (duty_type != 0) {
		    register_type = register_typeByduty_type(em.getTime(),duty_type);
		}
		if (register_type != null && !register_type.equals("false")) {
		    dao.insertAttendRecord(user_id, register_type,
			    em.getTime(), address, "", duty_type, 0, 0);
		    flg = em.getTime();
		}
	    }
	} catch (Exception e) {
	 ...
	 * 
	 */
    public String smpInsertDataToSql(List<OtherResponseDto> list, String address)
	    throws Exception {
	...
	try {
		...
		log.info(duty_type);
		if (duty_type != 0) {
		    register_type = register_typeByduty_type(em.getTime(),
			    duty_type);
		}
		if (register_type != null && !register_type.equals("false")) {
		    dao.insertAttendRecord(user_id, register_type,
			    em.getTime(), address, "", duty_type, 1, 0);
    ...
    @Override
    public String delectMinValueForPM(String user_id, String register_type,
	    String register_time) {
	String result = dao.deleteMinValueForPM(user_id, register_type,
		register_time);
	return result;
    }
	...
	if (list.size() > 0) {
	    ...
	    if (map.size() != 0) {
		Iterator<Entry<String, OtherOweAttendDto>> it2 = map.entrySet()
			.iterator();
		...
		while (it2.hasNext()) {
			...
			if (otherDto.getWork_no().equals(list.get(i).getId())
				&& (maxtime.getTime() < timer.getTime())) {
			    employ = new EmployDto(list.get(i).getTime(),
				    otherDto.getUser_id(), otherDto.getName(),
				    otherDto.getWork_no());
			    exlist.add(employ);
			}
		...
		Iterator<Entry<String, NewPerson>> it = mapN.entrySet()
			.iterator();
		while (it.hasNext()) {
    ...
    @Override
    public String insertToMysql(String attendIp, int port, String secretKey,
	    String charSet) throws Exception {

	String result = "";

	String answer = getAttDataWithoutStartTime(attendIp, port, secretKey,
		charSet);
	/*
	 * File file = new File("d:\\test.txt"); FileOutputStream fos = new
	 * FileOutputStream(file); OutputStreamWriter osw = new
	 * OutputStreamWriter(fos); BufferedWriter bw = new BufferedWriter(osw);
	 * bw.write(answer); bw.flush(); bw.close(); osw.close(); fos.close();
    ...
    // ///////////////////////////////////////////新版本//////////////////////////////////////////////
    @Override
    public AttendMachineNodeDto selectAttendMachineNodeDate(
	    String attendMachineCode) {

	AttendMachineNodeDto dto = dao
		.selectAttendMachineNodeDate(attendMachineCode);
	return dto;
    }
	...
	DutyIntervalDto dto = new DutyIntervalDto(after1, after2, before1,
		before2);
	return dto;
    }
	...
	try {
	    ...
	    if (list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
			    ...
			    String user_id = selectUseridByWorkNo(employee.getId());// 根据工号获取user_id
			    if (user_id != null && !"".equals(user_id)) {
				employ = new EmployDto(employee.getTime(), user_id,
				employee.getName(), employee.getId());
				exlist.add(employ);
			    }else{
			    	log.info("user_id is wrong...");
			    }
			}
	    }
	} catch (Exception e) {
    ...
    @Override
    public String getDataFormAttend(String attendIp, int port,
	    String secretKey, String charSet, String startTime) {
	String answer = null;
	try {
    ...
    public String register_typeByduty_type(String time, int duty_type) {
		...
		try {
		    ...
		    if (DateUtil.isBetween(time.substring(11), "00:00:00",moringMaxWork, "HH:mm:ss")) {
		    	num = "1";
		    } else if (DateUtil.isBetween(time.substring(11), moringMinOffWork,
			    moringMaxOFFWork, "HH:mm:ss")) {
			num = "2";
		    } else if (DateUtil.isBetween(time.substring(11), noonMinWork,
			    noonMaxWork, "HH:mm:ss")) {
			num = "3";
		    } else if (DateUtil.isBetween(time.substring(11), noonMinOffWork,
			    "23:59:59", "HH:mm:ss")) {
			num = "4";
		    } else {
			num = "5";
    ...
    @Override
    public String insertToMysql(String attendIp, int port, String secretKey,
	    String charSet, String startTime, String machineCode, String remark) {

    	String result = "";
    	String answer = getDataFormAttend(attendIp, port, secretKey, charSet,startTime);// 获取考勤数据
    	
    	if (!"getDataFormAttendBad".equals(answer)) {
    		List<EmployDto> list = encapsulateDataToList(answer);// 封装数据到list中
    		if (list.size() > 0) {
    			try {
    				result = insertEmployeeData(list, remark);
    				log.info(" step1: encapsulate Data To List size is " + list.size());
    			} catch (Exception e) {
    				log.info("make error : " + e);
    			}
    		} else {
		log.info(" step1: encapsulate Data To List size is zero");
	    }
	} else {
	...
	String answer = "";
	try {
	    fr = new FileReader("D:\\attend.txt");
	...
	// 可以换成工程目录下的其他文本文件
	// log.info("anser read"+answer);
	if (!"getDataFormAttendBad".equals(answer)) {
	    List<EmployDto> list = encapsulateDataToList(answer);// 封装数据到list中
	    if (list.size() > 0) {
		try {
		    result = insertEmployeeData(list, remark);
		    log.info(" step1: encapsulate Data To List size is "
			    + list.size());
		} catch (Exception e) {
		    log.info("make error : " + e);
		}
    ...
    @Override
    public boolean updateMachineLastTime(String time, String lastIp,
	    String machineCode) {
    ...
    public String deleteDuplData() {
    	String res = dao.deleteDuplData();
    	return res;
    }

    public static void main(String[] args) throws Exception {
    	AttendSynchServiceImpl service=new AttendSynchServiceImpl() ;
    	//service.insertToMysqlForTxt("天津");
    	String answer = service.getDataFormAttend("192.168.1.60", 9922, "2015", "gbk","2016-03-01");// 获取考勤数据
    	...
    	fos.close();
    	System.out.println("end.....");
    	service.encapsulateDataToList(answer);
