...
function pullUpAction () {
	...
	setTimeout(function () {	// <-- Simulate network congestion, remove setTimeout from production!
		$.ajax({
            url: "/mbem/mcrm/business/getZsCustomListByType.action",
            type:"POST",
            data: {"type":cstType,"page":pageIndex,"pageLen":pageLen,},
            dataType: "json",
            success: function (data) {
            	for(var i=0; i<data.length; i++) {
            		var str = str1+data[i].cusId+str2+data[i].cusName+str3+data[i].intentionType+str4+data[i].followDate+str5+data[i].followWayEach+str6+data[i].followInfo+str7+data[i].tel+str8;
            		$("#"+listId).append(str);
            	}
            	myScroll.refresh();
            	layer.close(index);
