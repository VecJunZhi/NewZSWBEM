	...
	<div class="widget-box">
		<div class="widget-title"><span class="icon"><i class="icon-th"></i></span><h5>问题列表</h5>
			...
			<div class="label" style="background-color: #EFEFEF;box-shadow:0 0 0 0;margin:0;">
				<shiro:hasPermission name="button:projectManage:issuesDeal:print"><button type="button" class="btn btn-success" onclick="print()">打印</button></shiro:hasPermission>
			</div>
         </div>
...
function repairFeedback(issuesId) {
	layer.open({
        type: 2,
        title: '修复确认',
        ...
        area : ['490px' , '300px'],
        content: "/wbem/inspect/repairFeedback.action?issuesId="+issuesId
    });
}
         ...
         "columnDefs":[
            {
	            targets:10,"render":function(data, type, row, meta){
	     			if(row.status == "工程修复完成" || row.status == "工程复修完成" || row.status == "物业确认完成") { 
	     			 return	 "<a style='color:#FAA732' href='#' onclick='issuesDetail(\"" + data + "\")'>查看</a>";
	       			}
	     			if(row.status == "未修复") {
	   					return "<shiro:hasPermission name='button:projectManage:issuesDeal:repairFeedback'><a style='color:#FAA732' href='#' onclick='repairFeedback(\"" + data + "\")'>修复完成</a></shiro:hasPermission>&nbsp;" + 
	       				 "<a style='color:#FAA732' href='#' onclick='issuesDetail(\"" + data + "\")'>查看</a>";	
	     			}
	     			if(row.status == "物业退回返修") {
	     				return "<shiro:hasPermission name='button:projectManage:issuesDeal:repairFeedback'><a style='color:#FAA732' href='#' onclick='repairFeedback(\"" + data + "\")'>复修完成</a></shiro:hasPermission>&nbsp;" + 
	       				 "<a style='color:#FAA732' href='#' onclick='issuesDetail(\"" + data + "\")'>查看</a>";
	     			} 
	     		}
         	}, 
