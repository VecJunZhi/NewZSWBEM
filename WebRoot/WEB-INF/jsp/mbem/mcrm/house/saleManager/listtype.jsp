...
<body>
    ...
    <article id="scrollArticle" class="scrollArticle" style="margin-top:-30px">
    	<div class="content">
        <div id="wrapper" class="follow" style="overflow: hidden;">
          ...
          <c:forEach items="${overduelistByYWY }" var="overdue" varStatus="status"> 
           		<div class="panel_group_title expand select border-1px">
                   <%--  <a href="/xs/performance/detail_listtype.action?userGuid=${overdue.userGuid}" >${overdue.userName} <span class="font_alert">${overdue.countNum}</span>位</a> --%>
                    ${overdue.userName} <span class="font_alert">${overdue.countNum}</span>位
                 </div>
                <div class="panel_group_content select hide" _index="0">
                    <ul> 
                    
	                    <c:forEach items="${overdueCustByYWY}" var="detail" varStatus="status">
	                 	 <%--   ${overdue.userGuid}</br>
	                   	 ${detail.userGuid}</br> --%>
	                   	 <c:if test="${overdue.userGuid == detail.key}">
	                   	   <%--  ${detail.key}</br> --%>
	                   	 	<c:forEach items="${overdueCustByYWY[detail.key]}" var="custom">
	                   	 		<li name="eachFollowItem">
						               <div class="a_li">
						                  ...
						                  <%--  <a href="/mbem/mcrm/house/customer/getXsCstAndFollowInfoByCstGuidAndZygw_Count.action?cusId=${custom.cstGuid}&zygw=${overdue.userName}" > --%> 
						                       <a href="${url }">
						                       <dl>
						                           <dt>
