package com.zs.common.entity;

import java.io.Serializable;

public class RoomTableEntity implements Serializable{
	
	private static final long serialVersionUID = 1L;
	private String roomGuid;//房间Guid
    private String buGuid;//单位Guid
    private String projGuid;//项目Guid
    private String bldGuid;//楼栋Guid
    private String mainRoomGuid;//主房间Guid
    private String unit;//单元
    private String floor;//楼层
    private String no;//号码
    private String room;//房号
    private String roomCode;//房间编码
    private String huXing;//户型
    private String status;//销售状态
    private String isVirtualRoom;//
    private String bldArea;//建筑面积
    private String tnArea;//套内面积
    private String blRhDate;//
    private String rhBlZt;
    private String yfbz;
    private String xPos;
    private String yPos;
    private String zxBz;
    private String price;
    private String tnPrice;//套内单价
    private String total;
    private String zxPrice;
    private String zxTotal;
    private String isTempletRoom;//样板房
    private String locker;
    private String lockTime;
    private String tfDate;
    private String djArea;//定位面积
    private String isAreaModify;
    private String virtualStatus;
    private String rhDate;
    private String jfrq;
    private String xkRow;
    private String xkCol;
    private String areaStatus;//面积状态
    private String west;//朝向
    private String areaChangingGuid;//
    private String importDate;
    private String chooseRoom;
    private String cstName;
    private String cstGuidList;
    private String chooseRoomCstName;
    private String chooseRoomDate;
    private String jbr;
    private String isAnnexe;
    private String sight;//景观
    private String roomStru;//房间结构
    private String absolutelyFloor;
    private String statusChgGuid;
    private String saleRentable;
    private String rentPrice;
    private String calcRentMode;
    private String rentUnit;
    private String bz;
    private String rentStatus;
    private String continueRentStatus;
    private String nextRentStatus;
    private String rentName;
    private String rentGuidList;
    private String rentStatusChgGuid;
    private String rentLocker;
    private String rentLockTime;
    private String rentamount;
    private String bProductTypeCode;
    private String ysBldArea;
    private String ysTnArea;
    private String scBldArea;
    private String scTnArea;
    private Integer chooseRoomNo;
    private String importDataSp5;
    private String floorNo;
    private String unitNo;
    private String slControlDate;
    private String isHfLock;
    private String priceDj;
    private String tnPriceDj;
    private String totalDj;
    private String isDj2AreaLock;
    private String isBzj2AreaLock;
    private String isDjTf;
    private String isBzjTf;
    private String isDjAreaodify;
    private String chooseRoomBookingGuid;
    private String roomInfo;
    private String dspBldArea;
    private String dspTnArea;
    private String dspAreaStatus;
    private String ipadReservestatus;
    private String isPreControl;
    private String preControlBy;
    private String preControlOn;
    private String projBatchGuid;
    private String chooseRoomCstGuid;
    private String bookingGuid;
    private String areaCreateOn;
    private String areaCreateBy;
    private String areaAuditOn;
    private String areaAuditBy;
    private String areaChangingOn;
    private String areaChangingBy;
    private String openingBatchGuid;
    private String createTime4Chk;
    private String scrmTimestampData;
    
	public String getRoomGuid() {
		return roomGuid;
	}
	public void setRoomGuid(String roomGuid) {
		this.roomGuid = roomGuid;
	}
	public String getBuGuid() {
		return buGuid;
	}
	public void setBuGuid(String buGuid) {
		this.buGuid = buGuid;
	}
	public String getProjGuid() {
		return projGuid;
	}
	public void setProjGuid(String projGuid) {
		this.projGuid = projGuid;
	}
	public String getBldGuid() {
		return bldGuid;
	}
	public void setBldGuid(String bldGuid) {
		this.bldGuid = bldGuid;
	}
	public String getMainRoomGuid() {
		return mainRoomGuid;
	}
	public void setMainRoomGuid(String mainRoomGuid) {
		this.mainRoomGuid = mainRoomGuid;
	}
	public String getUnit() {
		return unit;
	}
	public void setUnit(String unit) {
		this.unit = unit;
	}
	public String getFloor() {
		return floor;
	}
	public void setFloor(String floor) {
		this.floor = floor;
	}
	public String getNo() {
		return no;
	}
	public void setNo(String no) {
		this.no = no;
	}
	public String getRoom() {
		return room;
	}
	public void setRoom(String room) {
		this.room = room;
	}
	public String getRoomCode() {
		return roomCode;
	}
	public void setRoomCode(String roomCode) {
		this.roomCode = roomCode;
	}
	public String getHuXing() {
		return huXing;
	}
	public void setHuXing(String huXing) {
		this.huXing = huXing;
	}
	public String getStatus() {
		return status;
	}
	public void setStatus(String status) {
		this.status = status;
	}
	public String getIsVirtualRoom() {
		return isVirtualRoom;
	}
	public void setIsVirtualRoom(String isVirtualRoom) {
		this.isVirtualRoom = isVirtualRoom;
	}
	public String getBldArea() {
		return bldArea;
	}
	public void setBldArea(String bldArea) {
		this.bldArea = bldArea;
	}
	public String getTnArea() {
		return tnArea;
	}
	public void setTnArea(String tnArea) {
		this.tnArea = tnArea;
	}
	public String getBlRhDate() {
		return blRhDate;
	}
	public void setBlRhDate(String blRhDate) {
		this.blRhDate = blRhDate;
	}
	public String getRhBlZt() {
		return rhBlZt;
	}
	public void setRhBlZt(String rhBlZt) {
		this.rhBlZt = rhBlZt;
	}
	public String getYfbz() {
		return yfbz;
	}
	public void setYfbz(String yfbz) {
		this.yfbz = yfbz;
	}
	public String getxPos() {
		return xPos;
	}
	public void setxPos(String xPos) {
		this.xPos = xPos;
	}
	public String getyPos() {
		return yPos;
	}
	public void setyPos(String yPos) {
		this.yPos = yPos;
	}
	public String getZxBz() {
		return zxBz;
	}
	public void setZxBz(String zxBz) {
		this.zxBz = zxBz;
	}
	public String getPrice() {
		return price;
	}
	public void setPrice(String price) {
		this.price = price;
	}
	public String getTnPrice() {
		return tnPrice;
	}
	public void setTnPrice(String tnPrice) {
		this.tnPrice = tnPrice;
	}
	public String getTotal() {
		return total;
	}
	public void setTotal(String total) {
		this.total = total;
	}
	public String getZxPrice() {
		return zxPrice;
	}
	public void setZxPrice(String zxPrice) {
		this.zxPrice = zxPrice;
	}
	public String getZxTotal() {
		return zxTotal;
	}
	public void setZxTotal(String zxTotal) {
		this.zxTotal = zxTotal;
	}
	public String getIsTempletRoom() {
		return isTempletRoom;
	}
	public void setIsTempletRoom(String isTempletRoom) {
		this.isTempletRoom = isTempletRoom;
	}
	public String getLocker() {
		return locker;
	}
	public void setLocker(String locker) {
		this.locker = locker;
	}
	public String getLockTime() {
		return lockTime;
	}
	public void setLockTime(String lockTime) {
		this.lockTime = lockTime;
	}
	public String getTfDate() {
		return tfDate;
	}
	public void setTfDate(String tfDate) {
		this.tfDate = tfDate;
	}
	public String getDjArea() {
		return djArea;
	}
	public void setDjArea(String djArea) {
		this.djArea = djArea;
	}
	public String getIsAreaModify() {
		return isAreaModify;
	}
	public void setIsAreaModify(String isAreaModify) {
		this.isAreaModify = isAreaModify;
	}
	public String getVirtualStatus() {
		return virtualStatus;
	}
	public void setVirtualStatus(String virtualStatus) {
		this.virtualStatus = virtualStatus;
	}
	public String getRhDate() {
		return rhDate;
	}
	public void setRhDate(String rhDate) {
		this.rhDate = rhDate;
	}
	public String getJfrq() {
		return jfrq;
	}
	public void setJfrq(String jfrq) {
		this.jfrq = jfrq;
	}
	public String getXkRow() {
		return xkRow;
	}
	public void setXkRow(String xkRow) {
		this.xkRow = xkRow;
	}
	public String getXkCol() {
		return xkCol;
	}
	public void setXkCol(String xkCol) {
		this.xkCol = xkCol;
	}
	public String getAreaStatus() {
		return areaStatus;
	}
	public void setAreaStatus(String areaStatus) {
		this.areaStatus = areaStatus;
	}
	public String getWest() {
		return west;
	}
	public void setWest(String west) {
		this.west = west;
	}
	public String getAreaChangingGuid() {
		return areaChangingGuid;
	}
	public void setAreaChangingGuid(String areaChangingGuid) {
		this.areaChangingGuid = areaChangingGuid;
	}
	public String getImportDate() {
		return importDate;
	}
	public void setImportDate(String importDate) {
		this.importDate = importDate;
	}
	public String getChooseRoom() {
		return chooseRoom;
	}
	public void setChooseRoom(String chooseRoom) {
		this.chooseRoom = chooseRoom;
	}
	public String getCstName() {
		return cstName;
	}
	public void setCstName(String cstName) {
		this.cstName = cstName;
	}
	public String getCstGuidList() {
		return cstGuidList;
	}
	public void setCstGuidList(String cstGuidList) {
		this.cstGuidList = cstGuidList;
	}
	public String getChooseRoomCstName() {
		return chooseRoomCstName;
	}
	public void setChooseRoomCstName(String chooseRoomCstName) {
		this.chooseRoomCstName = chooseRoomCstName;
	}
	public String getChooseRoomDate() {
		return chooseRoomDate;
	}
	public void setChooseRoomDate(String chooseRoomDate) {
		this.chooseRoomDate = chooseRoomDate;
	}
	public String getJbr() {
		return jbr;
	}
	public void setJbr(String jbr) {
		this.jbr = jbr;
	}
	public String getIsAnnexe() {
		return isAnnexe;
	}
	public void setIsAnnexe(String isAnnexe) {
		this.isAnnexe = isAnnexe;
	}
	public String getSight() {
		return sight;
	}
	public void setSight(String sight) {
		this.sight = sight;
	}
	public String getRoomStru() {
		return roomStru;
	}
	public void setRoomStru(String roomStru) {
		this.roomStru = roomStru;
	}
	public String getAbsolutelyFloor() {
		return absolutelyFloor;
	}
	public void setAbsolutelyFloor(String absolutelyFloor) {
		this.absolutelyFloor = absolutelyFloor;
	}
	public String getStatusChgGuid() {
		return statusChgGuid;
	}
	public void setStatusChgGuid(String statusChgGuid) {
		this.statusChgGuid = statusChgGuid;
	}
	public String getSaleRentable() {
		return saleRentable;
	}
	public void setSaleRentable(String saleRentable) {
		this.saleRentable = saleRentable;
	}
	public String getRentPrice() {
		return rentPrice;
	}
	public void setRentPrice(String rentPrice) {
		this.rentPrice = rentPrice;
	}
	public String getCalcRentMode() {
		return calcRentMode;
	}
	public void setCalcRentMode(String calcRentMode) {
		this.calcRentMode = calcRentMode;
	}
	public String getRentUnit() {
		return rentUnit;
	}
	public void setRentUnit(String rentUnit) {
		this.rentUnit = rentUnit;
	}
	public String getBz() {
		return bz;
	}
	public void setBz(String bz) {
		this.bz = bz;
	}
	public String getRentStatus() {
		return rentStatus;
	}
	public void setRentStatus(String rentStatus) {
		this.rentStatus = rentStatus;
	}
	public String getContinueRentStatus() {
		return continueRentStatus;
	}
	public void setContinueRentStatus(String continueRentStatus) {
		this.continueRentStatus = continueRentStatus;
	}
	public String getNextRentStatus() {
		return nextRentStatus;
	}
	public void setNextRentStatus(String nextRentStatus) {
		this.nextRentStatus = nextRentStatus;
	}
	public String getRentName() {
		return rentName;
	}
	public void setRentName(String rentName) {
		this.rentName = rentName;
	}
	public String getRentGuidList() {
		return rentGuidList;
	}
	public void setRentGuidList(String rentGuidList) {
		this.rentGuidList = rentGuidList;
	}
	public String getRentStatusChgGuid() {
		return rentStatusChgGuid;
	}
	public void setRentStatusChgGuid(String rentStatusChgGuid) {
		this.rentStatusChgGuid = rentStatusChgGuid;
	}
	public String getRentLocker() {
		return rentLocker;
	}
	public void setRentLocker(String rentLocker) {
		this.rentLocker = rentLocker;
	}
	public String getRentLockTime() {
		return rentLockTime;
	}
	public void setRentLockTime(String rentLockTime) {
		this.rentLockTime = rentLockTime;
	}
	public String getRentamount() {
		return rentamount;
	}
	public void setRentamount(String rentamount) {
		this.rentamount = rentamount;
	}
	public String getbProductTypeCode() {
		return bProductTypeCode;
	}
	public void setbProductTypeCode(String bProductTypeCode) {
		this.bProductTypeCode = bProductTypeCode;
	}
	public String getYsBldArea() {
		return ysBldArea;
	}
	public void setYsBldArea(String ysBldArea) {
		this.ysBldArea = ysBldArea;
	}
	public String getYsTnArea() {
		return ysTnArea;
	}
	public void setYsTnArea(String ysTnArea) {
		this.ysTnArea = ysTnArea;
	}
	public String getScBldArea() {
		return scBldArea;
	}
	public void setScBldArea(String scBldArea) {
		this.scBldArea = scBldArea;
	}
	public String getScTnArea() {
		return scTnArea;
	}
	public void setScTnArea(String scTnArea) {
		this.scTnArea = scTnArea;
	}
	public Integer getChooseRoomNo() {
		return chooseRoomNo;
	}
	public void setChooseRoomNo(Integer chooseRoomNo) {
		this.chooseRoomNo = chooseRoomNo;
	}
	public String getImportDataSp5() {
		return importDataSp5;
	}
	public void setImportDataSp5(String importDataSp5) {
		this.importDataSp5 = importDataSp5;
	}
	public String getFloorNo() {
		return floorNo;
	}
	public void setFloorNo(String floorNo) {
		this.floorNo = floorNo;
	}
	public String getUnitNo() {
		return unitNo;
	}
	public void setUnitNo(String unitNo) {
		this.unitNo = unitNo;
	}
	public String getSlControlDate() {
		return slControlDate;
	}
	public void setSlControlDate(String slControlDate) {
		this.slControlDate = slControlDate;
	}
	public String getIsHfLock() {
		return isHfLock;
	}
	public void setIsHfLock(String isHfLock) {
		this.isHfLock = isHfLock;
	}
	public String getPriceDj() {
		return priceDj;
	}
	public void setPriceDj(String priceDj) {
		this.priceDj = priceDj;
	}
	public String getTnPriceDj() {
		return tnPriceDj;
	}
	public void setTnPriceDj(String tnPriceDj) {
		this.tnPriceDj = tnPriceDj;
	}
	public String getTotalDj() {
		return totalDj;
	}
	public void setTotalDj(String totalDj) {
		this.totalDj = totalDj;
	}
	public String getIsDj2AreaLock() {
		return isDj2AreaLock;
	}
	public void setIsDj2AreaLock(String isDj2AreaLock) {
		this.isDj2AreaLock = isDj2AreaLock;
	}
	public String getIsBzj2AreaLock() {
		return isBzj2AreaLock;
	}
	public void setIsBzj2AreaLock(String isBzj2AreaLock) {
		this.isBzj2AreaLock = isBzj2AreaLock;
	}
	public String getIsDjTf() {
		return isDjTf;
	}
	public void setIsDjTf(String isDjTf) {
		this.isDjTf = isDjTf;
	}
	public String getIsBzjTf() {
		return isBzjTf;
	}
	public void setIsBzjTf(String isBzjTf) {
		this.isBzjTf = isBzjTf;
	}
	public String getIsDjAreaodify() {
		return isDjAreaodify;
	}
	public void setIsDjAreaodify(String isDjAreaodify) {
		this.isDjAreaodify = isDjAreaodify;
	}
	public String getChooseRoomBookingGuid() {
		return chooseRoomBookingGuid;
	}
	public void setChooseRoomBookingGuid(String chooseRoomBookingGuid) {
		this.chooseRoomBookingGuid = chooseRoomBookingGuid;
	}
	public String getRoomInfo() {
		return roomInfo;
	}
	public void setRoomInfo(String roomInfo) {
		this.roomInfo = roomInfo;
	}
	public String getDspBldArea() {
		return dspBldArea;
	}
	public void setDspBldArea(String dspBldArea) {
		this.dspBldArea = dspBldArea;
	}
	public String getDspTnArea() {
		return dspTnArea;
	}
	public void setDspTnArea(String dspTnArea) {
		this.dspTnArea = dspTnArea;
	}
	public String getDspAreaStatus() {
		return dspAreaStatus;
	}
	public void setDspAreaStatus(String dspAreaStatus) {
		this.dspAreaStatus = dspAreaStatus;
	}
	public String getIpadReservestatus() {
		return ipadReservestatus;
	}
	public void setIpadReservestatus(String ipadReservestatus) {
		this.ipadReservestatus = ipadReservestatus;
	}
	public String getIsPreControl() {
		return isPreControl;
	}
	public void setIsPreControl(String isPreControl) {
		this.isPreControl = isPreControl;
	}
	public String getPreControlBy() {
		return preControlBy;
	}
	public void setPreControlBy(String preControlBy) {
		this.preControlBy = preControlBy;
	}
	public String getPreControlOn() {
		return preControlOn;
	}
	public void setPreControlOn(String preControlOn) {
		this.preControlOn = preControlOn;
	}
	public String getProjBatchGuid() {
		return projBatchGuid;
	}
	public void setProjBatchGuid(String projBatchGuid) {
		this.projBatchGuid = projBatchGuid;
	}
	public String getChooseRoomCstGuid() {
		return chooseRoomCstGuid;
	}
	public void setChooseRoomCstGuid(String chooseRoomCstGuid) {
		this.chooseRoomCstGuid = chooseRoomCstGuid;
	}
	public String getBookingGuid() {
		return bookingGuid;
	}
	public void setBookingGuid(String bookingGuid) {
		this.bookingGuid = bookingGuid;
	}
	public String getAreaCreateOn() {
		return areaCreateOn;
	}
	public void setAreaCreateOn(String areaCreateOn) {
		this.areaCreateOn = areaCreateOn;
	}
	public String getAreaCreateBy() {
		return areaCreateBy;
	}
	public void setAreaCreateBy(String areaCreateBy) {
		this.areaCreateBy = areaCreateBy;
	}
	public String getAreaAuditOn() {
		return areaAuditOn;
	}
	public void setAreaAuditOn(String areaAuditOn) {
		this.areaAuditOn = areaAuditOn;
	}
	public String getAreaAuditBy() {
		return areaAuditBy;
	}
	public void setAreaAuditBy(String areaAuditBy) {
		this.areaAuditBy = areaAuditBy;
	}
	public String getAreaChangingOn() {
		return areaChangingOn;
	}
	public void setAreaChangingOn(String areaChangingOn) {
		this.areaChangingOn = areaChangingOn;
	}
	public String getAreaChangingBy() {
		return areaChangingBy;
	}
	public void setAreaChangingBy(String areaChangingBy) {
		this.areaChangingBy = areaChangingBy;
	}
	public String getOpeningBatchGuid() {
		return openingBatchGuid;
	}
	public void setOpeningBatchGuid(String openingBatchGuid) {
		this.openingBatchGuid = openingBatchGuid;
	}
	public String getCreateTime4Chk() {
		return createTime4Chk;
	}
	public void setCreateTime4Chk(String createTime4Chk) {
		this.createTime4Chk = createTime4Chk;
	}
	public String getScrmTimestampData() {
		return scrmTimestampData;
	}
	public void setScrmTimestampData(String scrmTimestampData) {
		this.scrmTimestampData = scrmTimestampData;
	}

}
